...
public class QueryConnector implements Runnable {
	...
	public QueryConnector() {
		this.plugin = VelocitySpeak.getInstance();
		query = VelocitySpeak.getQuery();
	...
	public void run() {
		...
		if (channelEvents) {
			query.getApi().registerEvent(TS3EventType.CHANNEL, VelocitySpeak.getQueryInfo().getChannelId());
		}
		if (channelMessages) {
			query.getApi().registerEvent(TS3EventType.TEXT_CHANNEL, VelocitySpeak.getQueryInfo().getChannelId());
		}
		...
		plugin.setStartedTime(new Date());
		logger.info("Connected with SID = " + VelocitySpeak.getQueryInfo().getVirtualServerId() + ", CID = "
				+ VelocitySpeak.getQueryInfo().getChannelId() + ", CLID = " + VelocitySpeak.getQueryInfo().getId());
	}
}
